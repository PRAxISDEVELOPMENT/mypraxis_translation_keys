...
const { LOCALES } = require('../shared/constants');
const { readApplicationConfig, readNamespaceConfig } = require('../shared/config');
const { hasOwn, readSourceEntries, sortEntries, toOptionalTrimmedString } = require('../shared/entries');
const { readJsonFile, writeJsonFile } = require('../shared/json');
const { ROOT_DIR, SOURCE_PATH } = require('../shared/paths');
const { runBuild } = require('../shared/scripts');
const { createReportSkeleton } = require('./report');
const {
  buildSuggestedKey,
  ensureUniqueKey,
  getDefaultProposalApplications
} = require('./suggestions');
const { getUploadText } = require('./text');
...
function runProcessUploadCommand(argv = process.argv.slice(2)) {
  const { parseProcessUploadArgs, printProcessUploadHelp } = require('./processUploadArgs');
